    ...
    try {
      ...
      const baseSecretarias =
        secretarias.length > 0
          ? secretarias
          : SECRETARIAS_BASE.map((s, i) => ({ id_secretaria: null, nombre: s.nombre, orden: s.orden }));

      // Combinar: para cada secretaria buscamos si hay cargo activo en la gestión
      const filasCombinadas = baseSecretarias.map((sec) => {
        ...
        return {
          ...
          orden:          sec.orden,
          // datos del cargo (puede ser null si no hay asignación)
